...
! Parameters for implicit RK solver
  ...
  real(dl), parameter, dimension(nfld) :: dfld0 = (/0.,0./)
...
!
  subroutine evalf(ycur, yprime)
    ...
    yprime(nfld+1) = - 0.5*(ycur(3)**2*gp1 + ycur(4)**2*gp2) /acur**2 - acur**4*modeldv(ycur(1),ycur(2),1)
    yprime(2*nfld) = -acur**4*modeldv(ycur(1),ycur(2),2)
    yprime(2*nfld+1) = 0.
...
!$OMP PARALLEL DO FIRSTPRIVATE(b0,b1,b2,b3,yscl,dt)
    ...
    FLOOPEND
!$OMP END PARALLEL DO
    tmp_lat(SIRANGE) = (1.+zetafac*fld(1,SIRANGE)**2)*tmp_lat(SIRANGE)**2
!$OMP PARALLEL DO FIRSTPRIVATE(b0,b1,b2,b3,yscl,dt)
    FLOOP
...
!!$OMP PARALLEL DO PRIVATE(gp1,gp2,lfac,lap,grad2) FIRSTPRIVATE(b0,b1,b2,b3,d0,d1,d2,d3,yscl,dt) REDUCTION(+:GE2)
    FLOOP
      ...
      lfac(1) = (1.+zetafac*fld(1,LATIND)**2)*lfac(2)**2
      gp1 = 2.*fld(1,LATIND)*lfac(2)**2 * ( -2.*(1.+zetafac*fld(1,LATIND)**2)*zeta*lfac(2) + zetafac)  ! check this one
      gp2 = -2.*zeta*fld(1,LATIND)*lfac(2)**2

      grad2(1:nfld) = STENCIL(d,GRAD2)
      fldp(:,i,j,k) = fldp(:,LATIND) - yscl**2*dt*(gp1*grad2(1) + gp2*grad2(2)) 
...
!!$OMP PARALLEL DO PRIVATE(grad2,fac) FIRSTPRIVATE(z1,zfac,c0,c1,c2,c3) REDUCTION(+:GE)
    FLOOP
      fac(1) = 1./(1.+z1*fld(1,LATIND)**2)
      fac(2) = fac(1)**2*(1.+zfac*fld(1,LATIND)**2)
...
!$OMP PARALLEL DO PRIVATE(fac) FIRSTPRIVATE(z1,zfac) REDUCTION(+:PE,KE)
    FLOOP
      ...
      fac(2) = fac(1) / (1.+zfac*fld(1,LATIND)**2)
      KE = KE + fac(1)*(fac(2)*fldp(1,LATIND)**2 + fldp(2,LATIND)**2)
