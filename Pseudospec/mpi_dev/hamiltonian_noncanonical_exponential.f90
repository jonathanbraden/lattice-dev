...
! Scale laplacian coefficients to reduce number of multiplies
    ...
    tmp_lat(SIRANGE) = exp(alpha*fld(1,SIRANGE))
...
! This is generic and can be moved elsewhere
  ...
  function scalar_rho()
    ...
    egrad = 0.25/dx**2/cc
    GE = 0._dl
!!$OMP PARALLEL DO PRIVATE(grad2,fac) FIRSTPRIVATE(alph_loc,c0,c1,c2,c3) REDUCTION(+:GE)
    FLOOP
      fac(1) = 1.
      fac(2) = exp(alph_loc*fld(1,LATIND))
...
#ifdef LOOPEVOLVE
    KE = 0.; PE = 0.
!$OMP PARALLEL DO PRIVATE(fac) FIRSTPRIVATE(alph_loc) REDUCTION(+:PE,KE)
    FLOOP
      fac(1) = exp(-alph_loc*fld(1,LATIND))
