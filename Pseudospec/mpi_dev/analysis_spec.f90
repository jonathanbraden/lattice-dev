...
!  end interface init_spectrum
  ...
  integer, private :: nkx, nky, nkz, kz_start, kz_loc
  integer :: ns, ns1, ns2
...
contains
  ...
  subroutine init_spectrum_3d(nx, ny, nz, k_start, k_loc)
    integer(C_INTPTR_T), intent(in) :: nx, ny, nz, k_start, k_loc
...
!    plan = fftw_plan_dft_r2c_3d(nz,ny,nx,f,Fk,FFTW_ESTIMATE)
    ...
    if (present(plan)) then
       call fftw_mpi_execute_dft_r2c(plan,f,Fk)
    else
       plan2 = fftw_mpi_plan_dft_r2c_3d(nz,ny,nx,f,Fk,MPI_COMM_WORLD,FFTW_ESTIMATE)
       call fftw_mpi_execute_dft_r2c(plan2,f,Fk)
...
#ifdef USEMPI
    do k=1,z_size; k_loc=k+z_start_index; if (k_loc > nn3) then; kk=-(n3+1-k_loc); else; kk=k_loc-1; endif
#else
...
!$OMP END PARALLEL DO
  ...
  subroutine spectrum_2d(S, f, Fk, plan)
    ...
    if (present(plan)) then
       ...
       plan2 = fftw_mpi_plan_dft_r2c_2d(ny,nx,f,Fk,MPI_COMM_WORLD,FFTW_ESTIMATE)
       call fftw_mpi_execute_dft_r2c(plan2, f, Fk)
       call fftw_destroy_plan(plan2)
    endif
#else
...
#ifdef USEMPI
    call MPI_Allreduce(MPI_IN_PLACE, S_real, ns, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, mpierror)
    call MPI_Allreduce(MPI_IN_PLACE, S_imag, ns, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, mpierror)
    call MPI_Allreduce(MPI_IN_PLACE, W, ns, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, mpierror)
#endif
