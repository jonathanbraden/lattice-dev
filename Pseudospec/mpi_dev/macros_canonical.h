...
#define IRANGE 1:nx,1:ny,1:z_size
#define SIRANGE 0:nx+1,0:ny+1,0:z_size+1
#define LATIND i,j,k
#define NPTS dble(nx)*dble(ny)*dble(nz)
#define FLOOP do k=1,z_size; do j=1,ny; do i=1,nx
#define FLOOPEND enddo; enddo; enddo

#define LAPLACIAN(x,y,z) fld(:,i+(x),j+(y),k+(z))
#define GRAD_SQUARED(x,y,z) ( fld(:,i+(x),j+(y),k+(z)) - fld(:,i,j,k) )**2
