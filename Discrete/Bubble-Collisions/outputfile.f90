...
!!!!
...
#endif
#ifdef COMPUTEFOURMOMS
  integer, dimension(0:numkbins, 1:numbins, 1:maxfstats) :: PSD_DIST_AMP, PSD_DIST_PHASE, PSD_DIST_RE, PSD_DIST_IM
#endif
#ifdef COMPUTEPSD
  ...
  character(12), dimension(1:maxfstats) :: DVARF
...
!
    ...
    subroutine dump_outputbuffers( v, t, f, st, en )
      ...
      idx = idx + 1
      if (fouriermomflag .or. fourierdistflag) then
         idxf = idxf + 1
         DVARF(idxf) = trim(v)
      endif
    ...
    subroutine flush_outputbuffers(t, scl, hub, openfiles)
      ...
      if (writepsd) then
         ...
         if (.not.o) then
            ...
            call head(psdfilenum, (/"t  ","a  ","H   ","k   ", DVAR(1:idx)/))
         endif
         do k=1,numkbins
            write(psdfilenum, '(32g25.16e2)') t, scl, hub, (k-1)*dk, PSD(k,1:idx)
         enddo
         write(psdfilenum,'(g)')  "",""
         flush(psdfilenum)
      endif
      ...
      if (writecdf) then
         ...
         if (.not.o) then
            ...
            call head(cdffilenum, (/"t  ","percentile ", DVAR(1:idx)/))
         endif
         do k=1,n+1
            write(cdffilenum,'(32g25.16e2)') t, dble(k-1)/n, CDF(k,1:idx)
         enddo
         write(cdffilenum,'(g)') "",""
         flush(cdffilenum)
      endif
      ...
      if (writefourdist) then
         do k=1,4
            ...
            if (.not.o) then
               ...
               call head( fourdistfilenum(k), (/"t   ","value ", DVARF(1:idxf)/) )
            endif
         enddo
         do k=1,numbins
            write(fourdistfilenum(1),'(32g)') t, real(k-1)*6./dble(numbins), PSD_DIST_AMP(0,k,1:idxf)
            write(fourdistfilenum(2),'(32g)') t, real(k-1)*twopi/dble(numbins) - twopi/2._dl, PSD_DIST_PHASE(0,k,1:idxf)
            write(fourdistfilenum(3),'(32g)') t, real(k-1-numbins/2)*6./dble(numbins)+3./dble(numbins), PSD_DIST_RE(0,k,1:idxf)
            write(fourdistfilenum(4),'(32g)') t, real(k-1-numbins/2)*6./dble(numbins)+3./dble(numbins), PSD_DIST_IM(0,k,1:idxf)
         enddo
         do k=1,4
            write(fourdistfilenum(k), '(g)') "",""
         enddo
      endif
#endif
      ...
      if (writefourmom) then
         ...
         if (.not.o) then
            ...
            call head(fourmomfilenum, (/"t  ","k   ", DVARF(1:idxf)/))
         endif
         do k=1,numkbins
            write(fourmomfilenum,'(32g)') t, (k-1)*dk, ( (/PSD_MOMS(k,1,1:2,i), PSD_MOMS(k,2,1,i), PSD_MOMS(k,3,1:2,i), PSD_MOMS(k,4,1,i)/), i=1,idxf )
         enddo
         write(fourmomfilenum,'(g)') "",""
      endif
#endif
...
!
    subroutine get_fourier_stats(f, st, en, famp, fst, fen) !, S_weight, four_moms, nummoms, amps, phases, recomp, imcomp, nbins)
      ...
      real(dl), dimension(1:numkbins,1:nummoms, 1:2) :: four_moms
      integer, dimension(0:numkbins, 1:numbins) :: amps, phases, recomp, imcomp
...
!# preprocessor for spectral moments
      ...
      if (fouriermomflag) then
         ...
         if (mpirank.eq.0) then
            call MPI_reduce(MPI_IN_PLACE, four_moms, numk*nummoms*2, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, mpierror)
         else
            call MPI_reduce(four_moms, 0, numk*nummoms*2, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, mpierror)
         endif
...
!# preprocessor for spectral distributions (binned)
      ...
      if (fourierdistflag) then
         ...
         if (mpirank.eq.0) then
            PSD_DIST_AMP(:,:,idxf) = amps(:,:) 
            PSD_DIST_PHASE(:,:,idxf) = phases(:,:) 
            PSD_DIST_RE(:,:,idxf) = recomp(:,:)
            PSD_DIST_IM(:,:,idxf) = imcomp(:,:) 
         endif
      endif
#endif
...
!
    subroutine generating_function(f, st, en, slims, ssteps)
      ...
      genfunc = genfunc / rnvol   ! can make this more independent of knowledge of the cube (at expense of being slower?) 
...
!
    subroutine output_cube( v, frame, t, lat, st, en)  !, bovfn)
      ...
      write(buffer,'(a,a,a,i5.5,a)') 'output/',trim(v),'-',frame,'.raw'
      ...
      if (mpirank .eq. 0) then
         write (buffer,'(a,a,a,i5.5,a)') 'output/',trim(v),'-',frame,'.bov'
         open(bovfn,file=buffer)
         write(buffer,'(a,a,i5.5,a)') trim(v),'-',frame,'.raw'
!      endif
...
!      if (mpirank.eq.0) then
         ...
         write(bovfn,'(a,3g25.16e2)') 'BRICK_SIZE:  ', nside(1)*dx, nside(2)*dx, nside(3)*dx
         write(bovfn,'(2a)')          'DATA_FILE:   ', trim(buffer)
         write(bovfn,'(a,3i5)')       'DATA_SIZE:   ', nside(1)/ds, nside(2)/ds, nside(3)/ds
...
! Now make the BOV header file
    subroutine output_spec(v, frame, t, st, en, spec2d)
      character(*) :: v
      integer :: frame
      real(dl) :: t
      integer, dimension(1:2) :: st, en
      real(dl), dimension(st(1):en(1), st(2):en(2) ) :: spec2d
      ...
      datsize = (en(1)-st(1)+1)*(en(2)-st(2)+1)
      disp = mpirank*sizeof(spec2d(st(1),st(2)))*datsize

      bovfn = 99
  

      write(buffer,'(a,a,a,i5.5,a)') 'output/',trim(v),'spec-',frame,'.raw'
      call MPI_File_open(MPI_COMM_WORLD, buffer, MPI_MODE_WRONLY+MPI_MODE_CREATE, MPI_INFO_NULL, rawfh, mpierror)
      call MPI_file_set_view(rawfh, disp, MPI_DOUBLE_PRECISION, MPI_DOUBLE_PRECISION, 'native', MPI_INFO_NULL, mpierror)
      call MPI_file_write(rawfh, spec2d(st(1):en(1),st(2):en(2)), datsize, MPI_DOUBLE_PRECISION, MPI_STATUS_IGNORE, mpierror)
      call MPI_file_close(rawfh, mpierror)

      if (mpirank .eq. 0) then
         write(buffer,'(a,a,a,i5.5,a)') 'output/',trim(v),'spec-',frame,'.bov'
         open(bovfn, file=buffer)
         write(buffer,'(a,a,i5.5,a)') trim(v),'spec-',frame,'.raw'

         write(bovfn,'(a,g25.16e2)')  'TIME:        ', t
         write(bovfn,'(2a)')          'VARIABLE:    ', v
         write(bovfn,'(a,3g25.16e2)') 'BRICK_ORIGIN:', 0.0, 0.0, 0.0
         write(bovfn,'(a,3g25.16e2)') 'BRICK_SIZE:  ', ns_2*dk2_eff, nside(3)*dx, 0.
         write(bovfn,'(2a)')          'DATA_FILE:   ', trim(buffer)
         write(bovfn,'(a,3i5)')       'DATA_SIZE:   ', ns_2, nside(3), 1
         write(bovfn,'(2a)')          'DATA_FORMAT: ', "DOUBLE"
         write(bovfn,'(2a)')          'DATA_ENDIAN: ', "LITTLE"   ! bug check this, it depends on the processor
         write(bovfn,'(2a)')          'CENTERING:   ', "nodal"  !"zonal"    ! change if needed
!         write(bovfn,'(a,i5)')        'BYTE_OFFSET: ', 4    ! check if this is needed (since it's fortran) (don't need or visit screws up
         close(bovfn)
