...
!
    ...
    subroutine spectrum_bin_nofft(famp, S)
      ...
      do k=fstart(3),fend(3); if (k<=nn) then; kk=k-1; else; kk=n+1-k; endif
         do j=fstart(2),fend(2); if (j<=nn) then; jj=j-1; else; jj=n+1-j; endif
            do i=fstart(1),fend(1)-1; if (i<=nn) then; ii=i-1; else; ii=n+1-i; endif
...
! one-sided power spectrum density estimator (output valid only on rank-0 node)
    subroutine spectrum(f, S)
        ...
        where (W /= 0.0) S = S/W/dble(nvol)**2  
    end subroutine spectrum

!
! Get spectrum from a 2-D slice of data
!    
    subroutine spectrum_2d(f, st, en, S, numside)
      integer, dimension(1:2) :: st, en
      real(dl) :: f(st(1):en(1),st(2):en(2))
      real(dl), dimension(1:ns_2) :: S
      integer :: numside
...
! This needs to be made more general
      complex, dimension(numside/2+1,numside) :: fk_2
      integer*8 :: plan

      real(dl), dimension(1:ns_2) :: W
      integer :: nncur

      call dfftw_plan_dft_r2c_2d(plan, numside, numside, f, fk_2, FFTW_ESTIMATE)
      call dfftw_execute_dft_r2c(plan, f, fk_2)
...
! Worry about the periodigram stuff later
      do j=1,numside; if (j<=nncur) then; jj=j-1; else; jj = numside+1-j; endif
         do i=1,numside; if (i<=nncur) then; ii=i-1; else; ii=numside+1-i; endif
            p = sqrt(real(ii**2+jj**2)); l = floor(p)

            c = (1.0 - (/l-p, l+1-p/)**2)**2

            S(l+1:l+2) = S(l+1:l+2) + c*fk_2(ii+1,j)*conjg(fk_2(ii+1,j))
            W(l+1:l+2) = W(l+1:l+2) + c

!            S(l+1) = S(l+1) + fk_2(ii+1,j)*conjg(fk_2(ii+1,j))
!            W(l+1) = W(l+1) + 1.
         enddo
      enddo
            
      where (W /= 0.) S = S/W/dble(numside)**4
...
!
    subroutine crossspec(f1, f2, S1, S2)
      ...
      do k = fstart(3),fend(3); if (k <= nn) then; kk = k-1; else; kk = n+1-k; end if
         do j = fstart(2),fend(2); if (j <= nn) then; jj = j-1; else; jj = n+1-j; end if
            do i = 1,n;               if (i <= nn) then; ii = i-1; else; ii = n+1-i; end if
               ...
               Stmp(l+1:l+2) = Stmp(l+1:l+2) + c * Fk(ii+1,j,k)*conjg(Fk2(ii+1,j,k))
               W(l+1:l+2) = W(l+1:l+2) + c
            end do
         end do
      end do
      ...
      where (W/=0.0) S1 = S1/W/real(n)**6; S2=S2/(W*real(n)**6)
