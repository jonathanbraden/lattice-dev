...
!////////
  ...
  real(dl), parameter :: mpl= 470.   !1.e7/3.
  real(dl), parameter :: xi=100.
  real(dl), parameter :: xifac = xi*(1.+6.*xi)
...
! Parameters for implicit RK solver
  ...
  real(dl), parameter :: phiend = ( 0.5*(sqrt(1.+32.*xifac) - 1.) / xifac )**0.5
  real(dl), parameter :: dphiend = -2./3.**0.5*phiend / (1.+xifac*phiend**2)
  real(dl), parameter, dimension(nfld) :: fld0 = (/phiend,0./)
  real(dl), parameter, dimension(nfld) :: dfld0 = (/dphiend,0./)

! For approximate initial conditions, we just apply the differential normalization
  real(dl), parameter :: dcandphi = (1.+xifac*phiend**2)**0.5 / (1.+xi*phiend**2)
  real(dl), parameter :: dchidphi = 1./(1.+xi*phiend**2)**0.5
  real(dl), parameter, dimension(nfld) :: fld_norm = (/ 1./dcandphi, 1./dchidphi /)
  real(dl), parameter, dimension(nfld) :: fldp_norm = (/ dcandphi, dchidphi /)
  real(dl), parameter, dimension(nfld) :: m2eff = (/3.*fld0(1)**2,1./)
...
!
  subroutine evalf(ycur, yprime)
    ...
    kfac(1) = (1.+xi*ycur(1)**2) / (1. + xifac*ycur(1)**2)
    kfac(2) = 1.+xi*ycur(1)**2
    gp(1) = 4.*xi*ycur(1)*kfac(1) - 2.*xifac*ycur(1)*kfac(1)**2
    gp(2) = 2.*xi*ycur(1)
! Now correct
    kfac(1) = kfac(1) * kfac(2)
    ktmp = sum(kfac(1:nfld)*ycur(nfld+1:2*nfld)**2)

    yprime(1) = kfac(1)*ycur(3) / acur**2
    yprime(2) = kfac(2)*ycur(4) / acur**2
    yprime(nfld+1) = - 0.5*( sum(gp(1:nfld)*ycur(nfld+1:2*nfld)**2) ) /acur**2 - acur**4*modeldv(ycur(1),ycur(2),1)
...
!!$OMP PARALLEL DO PRIVATE(gp,lfac,lap,grad2) FIRSTPRIVATE(b0,b1,b2,b3,d0,d1,d2,d3,yscl,dt) REDUCTION(+:GE2)
    FLOOP
      lfac(2) = 1. / (1.+xi*fld(1,LATIND)**2)
      lfac(1) = (1.+xifac*fld(1,LATIND)**2)*lfac(2)**2
      gp(1) = 2.*fld(1,LATIND)*lfac(2)**2 * ( -2.*(1.+xifac*fld(1,LATIND)**2)*xi*lfac(2) + xifac )
      gp(2) = -2.*xi*fld(1,LATIND) * lfac(2)**2
...
! This is generic and can be moved elsewhere
  ...
  function scalar_rho()
    ...
    egrad = 0.25/dx**2/cc
    GE = 0._dl;  GE_i = 0._dl
!!$OMP PARALLEL DO PRIVATE(grad2,fac) FIRSTPRIVATE(xi_loc,xifac_loc,c0,c1,c2,c3) REDUCTION(+:GE)
    FLOOP
      fac(2) = 1./(1.+xi_loc*fld(1,LATIND)**2)
      fac(1) = (1.+xifac_loc*fld(1,LATIND)**2)*fac(2)**2
...
#ifdef LOOPEVOLVE
    KE = 0.; PE = 0.; KE_i=0.
!$OMP PARALLEL DO PRIVATE(fac) FIRSTPRIVATE(xi_loc,xifac_loc) REDUCTION(+:PE,KE)
    FLOOP
      fac(2) = 1.+xi_loc*fld(1,LATIND)**2
      fac(1) = fac(2)**2 / (1.+xifac_loc*fld(1,LATIND)**2)
      KE_i(1:nfld) = KE_i(1:nfld) + fac(1:nfld)*fldp(1:nfld,LATIND)**2
      KE = KE + fac(1)*fldp(1,LATIND)**2 + fac(2)*fldp(2,LATIND)**2
