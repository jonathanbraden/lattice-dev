...
!!!!
  ...
  integer, parameter :: nfdistbins = 4
  ...
  integer, dimension(1:nfdistbins, 1:numbins, 1:maxfstats) :: PSD_DIST_AMP, PSD_DIST_PHASE, PSD_DIST_RE, PSD_DIST_IM
#endif
#ifdef COMPUTEPSD
...
!
    subroutine flush_outputbuffers(t, scl, hub, openfiles)
      ...
      if (writefourdist) then
         ...
         enddo
         do k=1,numbins
            write(fourdistfilenum(1),'(32g)') t, real(k-1)*6./dble(numbins), ( (/PSD_DIST_AMP(:,k,i)/), i=1,idxf )
            write(fourdistfilenum(2),'(32g)') t, real(k-1)*twopi/dble(numbins) - twopi/2._dl, ( (/PSD_DIST_PHASE(:,k,i)/), i=1,idxf)
            write(fourdistfilenum(3),'(32g)') t, real(k-1-numbins/2)*6./dble(numbins)+3./dble(numbins), ( (/PSD_DIST_RE(:,k,i)/), i=1,idxf )
            write(fourdistfilenum(4),'(32g)') t, real(k-1-numbins/2)*6./dble(numbins)+3./dble(numbins), ( (/PSD_DIST_IM(:,k,i)/), i=1,idxf )
         enddo
         do k=1,4
...
!
    subroutine get_fourier_stats(f, st, en, famp, fst, fen) !, S_weight, four_moms, nummoms, amps, phases, recomp, imcomp, nbins)
      ...
      integer, dimension(1:nfdistbins, 1:numbins) :: amps, phases, recomp, imcomp
...
! I think this is the correct cycle
               ...
               kint = floor(keff) + 1
               l = kint - 1
               kdbin = ( (nfdistbins-1)* kint**3) / nn**3 + 1   ! divide bins to have equal numbers of modes in them
               if (kdbin .gt. nfdistbins) kdbin = nfdistbins
...
!
! Write out the data cube into a BOV file
!
