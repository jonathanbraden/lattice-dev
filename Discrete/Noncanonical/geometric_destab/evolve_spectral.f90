...
! Time Stepping Properties
  ...
  integer, parameter :: stepsize = 2**6
...
!      do j=2, ny-1
    ...
    subroutine init_fields(vparam)
      ...
      call sample(-0.25, m2eff(1))
      fld(1,IRANGE) = fld0(1) + fld_norm(1)*laplace(IRANGE)
      call sample(0.25, m2eff(1))
      fldp(1,IRANGE) = dfld0(1) + fldp_norm(1)*laplace(IRANGE)
      call sample(-0.25, m2eff(2))
      fld(2,IRANGE) = fld0(2) + fld_norm(2)*laplace(IRANGE)
      call sample(0.25, m2eff(2))
      fldp(2,IRANGE) = dfld0(2) + fldp_norm(2)*laplace(IRANGE)
...
!$OMP END PARALLEL DO
      ...
      GE = grad_energy()
      PE = potential_energy()
      KE_i = kinetic_energy_i()
      GE_i = grad_energy_i()

      if (mpirank == 0) then
#ifdef THREEDIM
      write(98,'(30(ES22.15,2X))') time, acur, rho, KE, PE, GE, grav_energy(), &
           (rho+grav_energy())/rho, -ysclp**2/12._dl/acur**4, KE_i, GE_i,&
...
!      call write_fields(time)
      ...
      call spectrum_3d(spec(:,3), laplace, Fk, planf)
      laplace(IRANGE) = fldp(2,IRANGE)
      call spectrum_3d(spec(:,4), laplace, Fk, planf)
#endif
#ifdef TWODIM
      ...
      if (mpirank == 0) then
         do i=1,ns
            write(97,'(30(ES22.15,2x))') (i-1)*dk, spec(i,1:4)
         enddo
         write(97,*)
      endif
