...
! Incorporate these below, and turn them into macros
#define KINMET(f) (1._dl+2._dl*(f)**2)
#define POT(f1,f2) (1._dl-exp(-mu*f1))**2 + 0.5*f2**2
...
!////////
  ...
  real(dl), parameter :: mpl= 1._dl
  real(dl), parameter :: mpl2 = mpl**2
  real(dl), parameter :: m2chi = 1._dl, m2kin = 2._dl*(1._dl/1.e-2)**2*mpl
  real(dl), parameter :: mu = (2./3.)**0.5
...
! Parameters for implicit RK solver
  ...
  ! Fix all this stuff
  real(dl), parameter :: phi0 = 2.3393837654714997732962993666073_dl
  real(dl), parameter :: dphi0 = -2.7363582010758065274616992909302_dl
  real(dl), parameter :: chi0 = 0._dl
  real(dl), parameter :: H0 = 1.9348974397391251388968698880012_dl
  real(dl), parameter, dimension(nfld) :: fld0 = (/phi0,0._dl/)
  real(dl), parameter, dimension(nfld) :: dfld0 = (/dphi0,0._dl/)
...
#ifdef THREEDIM
!  real(dl), parameter :: c3=0._dl, c2=0._dl, c1=1._dl, c0=-6._dl, cc=_dl
!  real(dl), parameter :: c3=0._dl, c2= _dl, c1= _dl, c0= _dl, cc=0._dl
...
!  real(dl), parameter :: c2=0._dl, c1=1._dl, c0=-4._dl
  ...
  real(dl), dimension(1:nfld), private :: kecur_i, gecur_i
...
! Iterate to get an approximation for the derivative
  ...
  !>@brief
  !> Basic equations of motion that will work for any diagonal field space metric with appropriate definitions of kfac and gp
  subroutine evalf(ycur, yprime)
    real(dl), dimension(1:nvar) :: ycur, yprime
    real(dl) :: acur, kfac(1:nfld), ktmp, gp(1:nfld)

    acur = ycur(2*nfld+1)
    
    kfac(1:2) = (/ 1._dl/(1._dl + m2kin*ycur(2)**2), 1._dl /)
    gp(1:2) = (/ -2._dl*m2kin*ycur(2)*kfac(1)**2, 0._dl /)
    
    ktmp = sum(kfac(1:nfld)*ycur(nfld+1:2*nfld)**2)

    yprime(1:nfld) = kfac(1:nfld)*ycur(nfld+1:2*nfld) / acur**2
    yprime(nfld+1) = -acur**4*modeldv(ycur(1),ycur(2),1)
    yprime(nfld+2) = 0.5_dl*( sum(gp(1:nfld)*ycur(nfld+1:2*nfld)**2) )/acur**2 - acur**4**4*modeldv(ycur(1),ycur(2),2)
    yprime(2*nfld+1) = 0._dl
    yprime(2*nfld+2) = ktmp / acur**3 - 4._dl*ycur(2*nfld+1)**3*potential(ycur(1),ycur(2))
  ...
  subroutine Hamiltonian_fields_grad_pot(dt)
    ...
    real(dl) :: fac(1:nfld), gp(1:nfld), lfac(nfld), grad2(nfld)
...
! Scale laplacian coefficients to reduce number of multiplies
    b0=0.5*c0*elap; b1=0.5*c1*elap; b2=0.5*c2*elap; b3=0.5*c3*elap  ! the 0.5 are in here to do the averaging in the GDOTL stencil
    d0=0.25*c0*elap; d1=0.25*c1*elap; d2=0.25*c2*elap; d3=0.25*c3*elap
    call wrap_fields()

    tmp_lat(SIRANGE) = 1._dl 
!$OMP PARALLEL DO FIRSTPRIVATE(b0,b1,b2,b3,yscl,dt)
    FLOOP
      fldp(2,LATIND) = fldp(2,LATIND) + yscl**2*dt*( STENCIL(b,GDOTL2) )
    FLOOPEND
!$OMP END PARALLEL DO
    tmp_lat(SIRANGE) = (1._dl + m2kin*fld(2,SIRANGE)**2)
!$OMP PARALLEL DO FIRSTPRIVATE(b0,b1,b2,b3,yscl,dt)
    FLOOP
...
!$OMP END PARALLEL DO
    ...
    GE2 = 0._dl
!!$OMP PARALLEL DO PRIVATE(gp,lfac,lap,grad2) FIRSTPRIVATE(b0,b1,b2,b3,d0,d1,d2,d3,yscl,dt) REDUCTION(+:GE2)
    FLOOP
      lfac(2) = 1._dl
      lfac(1) = (1._dl+m2kin*fld(2,LATIND)**2)
      ! Where are the 0.5 factors?  Already in the coeffs for the stencil?
      gp(1) = 2._dl*m2kin*fld(2,LATIND)
      gp(2) = 0._dl

      grad2(1:nfld) = STENCIL(d,GRAD2)
      fldp(2,LATIND) = fldp(2,LATIND) - yscl**2*dt*( gp(1)*grad2(1) )
!      fldp(1,i,j,k) = fldp(1,LATIND) - yscl**2*dt*(gp(1)*grad2(1) + gp(2)*grad2(2)) 
      GE2 = GE2 + sum(lfac(1:nfld)*grad2(1:nfld))
    FLOOPEND
!!$OMP END PARALLEL DO
#endif
  ...
  elemental function potential(f1,f2)
    ...
    real(dl), parameter :: eps = 0.1
    
    potential = V0*(1._dl-exp(-mu*f1))**2 + 0.5*m2chi*f2**2
  end function potential

  ! Fix this to not have a bunch of ugly if statements.  Should speed up code.
  elemental function modeldv(f1,f2,ind)
    ...
    if (ind==1) then
       modeldv = 2._dl*V0*mu*(1._dl-exp(-mu*f1))*exp(-mu*f1) 
    elseif (ind==2) then
       modeldv = m2chi*f2
    endif
  ...
  subroutine set_ke_i(kenew)
    real(dl), dimension(nfld) :: kenew
    kecur_i = kenew
  end subroutine set_ke_i
  subroutine set_ge_i(genew)
    real(dl), dimension(nfld) :: genew
    gecur_i = genew
  end subroutine set_ge_i
  subroutine set_ke(kenew)
    real(dl) :: kenew
    kecur = kenew
  end subroutine set_ke
  function kinetic_energy_i()
    real(dl), dimension(1:nfld) :: kinetic_energy_i
    kinetic_energy_i = kecur_i
  end function kinetic_energy_i
  function grad_energy_i()
    real(dl), dimension(1:nfld) :: grad_energy_i
    grad_energy_i = gecur_i
  end function grad_energy_i
...
! This is generic and can be moved elsewhere
  ...
  ! Fix this ugly beast as well
  function scalar_rho()
    real(dl) :: scalar_rho
    real(dl) :: GE, PE, KE, rho
    real(dl), dimension(nfld) :: GE_i, KE_i
    ...
    egrad = 0.25/dx**2/cc
    GE = 0._dl;  GE_i = 0._dl
!!$OMP PARALLEL DO PRIVATE(grad2,fac) FIRSTPRIVATE(c0,c1,c2,c3) REDUCTION(+:GE)  ! Seems to be missing reduction of GE_i
    FLOOP
      fac(1) = 1._dl + 2._dl*m2kin*fld(2,LATIND)**2
      fac(2) = 1._dl
      grad2(1:nfld) = STENCIL(c,GRAD2)
      GE_i(1:nfld) = GE_i(1:nfld) + fac(1:nfld)*grad2(1:nfld)
...
#ifdef LOOPEVOLVE
    KE = 0.; PE = 0.; KE_i=0.
!$OMP PARALLEL DO PRIVATE(fac) REDUCTION(+:PE,KE)  ! Seems to be missing reduction of KE_i
    FLOOP
      fac(2) = 1._dl
      fac(1) = 1._dl/(1._dl + 2._dl*m2kin*fld(2,LATIND)**2)
      KE_i(1:nfld) = KE_i(1:nfld) + fac(1:nfld)*fldp(1:nfld,LATIND)**2
      KE = KE + sum(fac(1:nfld)*dflp(1:nfld,LATIND)**2)  ! Redundant with above, can sum after
      PE = PE + potential(fld(1,LATIND),fld(2,LATIND))
    FLOOPEND
!$OMP END PARALLEL DO
#endif

    KE_i = 0.5*KE_i * kinetic_norm() / nvol
    GE_i = GE_i * egrad / nvol / yscl**2
    ...
    call MPI_Allreduce(MPI_IN_PLACE,KE_i,nfld,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,mpierror)
    call MPI_Allreduce(MPI_IN_PLACE,GE_i,nfld,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,mpierror)
#endif
