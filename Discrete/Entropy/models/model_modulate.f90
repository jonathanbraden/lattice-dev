  ...
  contains
    ...
    subroutine modelsummary(fd)
      ...
      write (fd,'(a,3(f0.5,a))') "# V(phi,psi) = ", &
           ...
           sqrt(g2), "^2*phi^2*psi^2/2 + modulation"
    ...
    function modelv(hr, i, j, k)
      ...
      modelv = (m2phi + g2*hr(sigma,i,j,k)**2*hr(psi,i,j,k)**2)*hr(phi,i,j,k)**2 + m2psi*hr(psi,i,j,k)**2 + m2sig*hr(sigma,i,j,k)**2
    ...
    function modeldv(hr, i, j, k)
      ...
      modeldv = (m2phi + g2*hr(sigma,i,j,k)**2*hr(psi,i,j,k)**2)*hr(phi,i,j,k)*V1  &
              + (m2psi + g2*hr(sigma,i,j,k)**2*hr(phi,i,j,k)**2)*hr(psi,i,j,k)*V2  &
              + (m2sig + g2*hr(phi,i,j,k)**2*hr(psi,i,j,k)**2)*hr(sigma,i,j,k)*V3
...
!      modelm2eff = (/ 0.0, g2*phi0**2 /)
      modelm2eff = (/ m2phi, m2psi + g2*phi0**2*sig0**2, m2sig + 0./)
      return
