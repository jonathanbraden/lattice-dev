...
#include "macros.h"
  ...
  contains
    ...
    function modelv(hr, i, j, k)
      ...
      modelv = 0.5*lambda*hr(phi,i,j,k)**4 + g2*hr(phi,i,j,k)**2*hr(psi,i,j,k)**2
    ...
    function modeldv(hr, i, j, k)
      ...
      modeldv =  (lambda*hr(phi,i,j,k)**2+g2*hr(psi,i,j,k)**2)*hr(phi,i,j,k)*V1 +  (g2*hr(phi,i,j,k)**2)*hr(psi,i,j,k)*V2
...
!      modelm2eff = (/ m2phi, m2psi + g2*phi0**2/)
      ...
      modelm2eff = (/ 3.*lambda*phi0**2+g2*psi0**2, g2*phi0**2 /)
...
!      m2eff(psi) = g2*fldave(phi)**2 + g2*fldvar(phi)**2
      ...
      m2eff(phi) = 3.*lambda*(fldave(phi)**2+fldvar(phi)**2) + g2*(fldave(psi)**2+fldvar(psi)**2)
      m2eff(psi) = g2*fldave(phi)**2 + g2*fldvar(phi)**2
...
!      potave = 0.5*m2phi*avefld(phi)**2 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
      ...
      potave = 0.25*lambda*avefld(phi)**4 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
...
!      potavefull = 0.5*m2phi*avefld(phi)**2 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
      ...
      potavefull = 0.25*lambda*avefld(phi)**4 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
...
!      fvprime(2,2) = g2*fval(1)**2*fval(2)**2
      ...
      fvprime(1,1) = lambda*fval(1)**4 + g2*fval(2)**2*fval(1)**2
      fvprime(1,2) = g2*fval(1)**3*fval(2)
      fvprime(2,1) = lambda*fval(1)**3*fval(2) + g2*fval(1)*fval(2)**3
      fvprime(2,2) = g2*fval(1)**2*fval(2)**2
