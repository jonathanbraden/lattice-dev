...
#include "macros.h"
  ...
  contains
    ...
    function modelv(hr, i, j, k)
      ...
      modelv = (m2phi + g2*hr(psi,i,j,k)**2)*hr(phi,i,j,k)**2 + m2psi*hr(psi,i,j,k)**2
    end function modelv
    ...
    function modeldv(hr, i, j, k)
      ...
      modeldv = (m2phi + g2*hr(psi,i,j,k)**2)*hr(phi,i,j,k)*V1 + &
                (m2psi + g2*hr(phi,i,j,k)**2)*hr(psi,i,j,k)*V2
...
!    
    function potave(avefld)
      ...
      potave = 0. !0.5*m2phi*avefld(phi)**2
...
!      potave = 0.5*m2phi*avefld(phi)**2 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
    ...
    function potavefull(avefld)
      ...
      potavefull = 0.
     !0.5*m2phi*avefld(phi)**2 + 0.5*g2*avefld(phi)**2*avefld(psi)**2
