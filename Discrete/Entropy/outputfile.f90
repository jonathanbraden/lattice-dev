...
!!!!
  ...
  integer, parameter :: nfdistbins = 5   ! number of bins less than nyquist (total number of bins then adds one for the nyquist
  integer, parameter :: maxstats = 2*fields + 2*(fields)*(2*fields-1) + 4
  integer, parameter :: maxfstats = fields + 2
  integer, parameter :: numbins = n/2
  integer, parameter :: nummoms = 4
  real(dl), parameter :: mombinfrac = dble( nfdistbins ) / dble(nn-1)

#ifdef COMPUTEFOURMOMS
  real(dl), dimension(1:2, 1:(nummoms+2), 1:numkbins, 1:maxfstats) :: PSD_MOMS
#endif
  ...
  character(namelength), dimension(1:maxstats) :: DVAR, DVARCDF
...
!
    ...
    subroutine dump_crossspec( v, t, famp1, famp2 )
      character(*) :: v
      real(dl) :: t
      complex, dimension(fstart(1):fend(1),fstart(2):fend(2),fstart(3):fend(3)) :: famp1,famp2

      real(dl), dimension(ns) :: respec, imspec

! Since these are fourier stats, I better iterate the power spectrum counter
      idx=idx+1
      DVAR(idx) = trim(v)
      idx=idx+1
      DVAR(idx) = trim(v)

      call crossspec_nofft(famp1, famp2, respec, imspec)
      PSD(:,idx-1) = respec(:)
      PSD(:,idx) = imspec(:)

    end subroutine dump_crossspec
    ...
    subroutine flush_outputbuffers(t, scl, hub, openfiles)
      ...
      if (writecdf) then
         ...
         if (.not.o) then
            ...
            call head(cdffilenum, [character(len=namelength) :: "t  ","percentile ", DVARCDF(1:idxcdf)])
         endif
         do k=1,n+1
            write(cdffilenum,outformat) t, dble(k-1)/n, CDF(k,1:idxcdf)
         enddo
         write(cdffilenum,'(a)') "",""
         flush(cdffilenum)
      endif
      ...
      if (writefourmom) then
         ...
         if (.not.o) then
            ...
            call head(fourmomfilenum, [character(len=namelength) :: "t  ","k   ","(RE,IM)**n","<RE*IM>","<|f|^2>","<RE**2*IM**2>","<|f|^4>", DVARF(1:idxf)])
         endif
         do k=1,numkbins
            write(fourmomfilenum,outformat) t, (k-1)*dk, ( (/ PSD_MOMS(1:2,1:(nummoms+2),k,i) /), i=1,idxf)
         enddo
...
!
    subroutine get_fourier_stats(f, st, en, famp, fst, fen) !, S_weight, four_moms, nummoms, amps, phases, recomp, imcomp, nbins)
      ...
      real(dl), dimension(1:2,1:(nummoms+2),1:numkbins) :: four_moms
...
! I think this is the correct cycle
               ...
               l = kint - 1
               if (kint.gt.1) then
                  kdbin = floor( keff * mombinfrac ) + 1  ! divide bins in equal increments of k**2*ln(k) to get approximately equal numbers of modes per bin?
...
!# preprocessor for getting PSD
               ...
               if (fouriermomflag) then
                  ...
                  ik = aimag(famp(i,j,k))
                  do m=1,nummoms
                     four_moms(1,m,kint) = four_moms(1,m,kint) + rk**m
                     four_moms(2,m,kint) = four_moms(2,m,kint) + ik**m
                  enddo
                  four_moms(1, nummoms+1, kint) = four_moms(1, nummoms+1, kint) + rk*ik
                  four_moms(2, nummoms+1, kint) = four_moms(2, nummoms+1, kint) + amp2
                  four_moms(1, nummoms+2, kint) = four_moms(1, nummoms+2, kint) + (rk*ik)**2
                  four_moms(2, nummoms+2, kint) = four_moms(2, nummoms+2, kint) + amp2**2
               endif
#endif
...
!         endif
         call MPI_ALLreduce(MPI_IN_PLACE, four_moms, numk*(nummoms+2)*2, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, mpierror)
!         if (mpirank.eq.0) then
...
#ifdef COMPUTEFOURDIST
      if (fourierdistflag) then
         call MPI_Allreduce(MPI_IN_PLACE, amps, (nfdistbins+1)*nbins, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, mpierror)
!         if (mpirank.eq.0) then
...
!         endif
         call MPI_Allreduce(MPI_IN_PLACE, phases, (nfdistbins+1)*nbins, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, mpierror)
!         if (mpirank.eq.0) then
...
!         endif
         ...
         call MPI_allreduce(MPI_IN_PLACE, recomp, (nfdistbins+1)*nbins, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, mpierror)
!         if (mpirank.eq.0) then
...
!         endif
         call MPI_allreduce(MPI_IN_PLACE, imcomp, (nfdistbins+1)*nbins, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, mpierror)
!         if (mpirank.eq.0) then
